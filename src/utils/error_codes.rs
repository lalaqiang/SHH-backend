...
mod tests {
    ...
    fn test_error_codes_naming_convention() {
        ...
        let codes = [
            ...
            AUTH_USER_NOT_FOUND, AUTH_PASSWORD_WRONG, AUTH_PASSWORD_TOO_SHORT,
            AUTH_PASSWORD_SAME, PERM_DENIED, PERMISSION_DENIED_TABLE, PERMISSION_DENIED_RECORD,
            RATE_LIMITED, STOCK_INSUFFICIENT,
            BIZ_DOC_STATE_INVALID, BIZ_DOC_ALREADY_APPROVED, BIZ_DOC_REVERSE_FAILED,
            BIZ_DOC_NOT_FOUND, BIZ_DOC_DETAIL_EMPTY, BIZ_DOC_NO_GEN_FAILED,
            VALIDATION_FIELD_REQUIRED, VALIDATION_FIELD_FORMAT,
            VALIDATION_TABLE_INVALID, VALIDATION_FIELD_INVALID, VALIDATION_PK_MISSING,
            SYS_DB_ERROR, SYS_DB_UNAVAILABLE, SYS_INTERNAL_ERROR, BAD_REQUEST,
            SYS_HASH_FAILED,
        ];
        for code in codes {
            assert!(
